...
import FormModal from "@/components/modules/ClassModule";
...
const ClassesListPage=()=>{
    ...
    const [selectedParent, setSelectedParent] = useState<clas | null>(null);
  
    const handleEdit = (clas: clas) => {
      setSelectedParent(clas);
      setOpenModal(true);
    }
  ...
  const [add,setadd]=useState(false);
    ...
    <FormModal open={openModal} onClose={() => setOpenModal(false)} clas={selectedParent} />

        {/* top */}

        <div className="">
        <h1 className="text-white mb-3 ml-10 text-[25px]">All Classes</h1>
        ...
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            ...
            <TableHead  className="text-left">Grade</TableHead>
              <TableHead  className="text-left">Subervisor</TableHead>
            ...
            <TableRow key={item.id}>
              ...
              <TableCell>
                <div className="flex items-center gap-3">
                  <div>
                    <div className="font-medium ml-5">{item.name}</div>

                  </div>
                </div>
              </TableCell>
             <TableCell><div className="ml-5">{item.capacity}</div></TableCell>
           <TableCell> <div className="ml-5">{item.grade}</div></TableCell>
