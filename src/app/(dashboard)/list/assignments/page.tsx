...
import FormModal from "@/components/modules/AssignmentsModule";
...
const AssignmentsListPage=()=>{
    ...
    const [selectedParent, setSelectedParent] = useState<assignmentsData | null>(null);
  
    const handleEdit = (assignmentsData: assignmentsData) => {
      setSelectedParent(assignmentsData);
      setOpenModal(true);
    }
...
const[add,setadd]=useState(false);
 ...
 <FormModal open={openModal} onClose={() => setOpenModal(false)} assignmentsData={selectedParent} />

        {/* top */}

        <div className="">
        <h1 className="text-white mb-3 ml-10 text-[25px]">All Assignments</h1>
            ...
            <TableRow key={item.id}>
              ...
              <TableCell>
                <div className="flex items-center gap-3">
                  <div>
                    <div className="font-medium ml-4">{item.subject}</div>

                  </div>
                </div>
              </TableCell>
             <TableCell><div className="ml-3">{item.class}</div></TableCell>
           <TableCell> <div className="ml-0">{item.teacher}</div></TableCell>
