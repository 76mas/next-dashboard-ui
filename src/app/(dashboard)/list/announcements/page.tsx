...
import FormModal from "@/components/modules/AnnouncementsModule";
...
const AnnouncementsListPage=()=>{
  const [openModal, setOpenModal] = useState(false);
        const [selectedParent, setSelectedParent] = useState<announcementsData | null>(null);
      
        const handleEdit = (announcementsData: announcementsData) => {
          setSelectedParent(announcementsData);
          setOpenModal(true);
        }
 ...
 <FormModal open={openModal} onClose={() => setOpenModal(false)} announcementsData={selectedParent} />
        {/* top */}

        <div className="">
        <h1 className="text-white mb-3 ml-10 text-[25px]">All Announcements</h1>
        ...
        <TableBody>
          {announcementsData.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell><div className="ml-0">{item.class}</div></TableCell>
