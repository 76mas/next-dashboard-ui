...
import FormModal from "@/components/modules/ResultModule";
import {
  Table,
  TableBody,
  TableCell,
...
const ResultsListPage=()=>{
  ...
  const [openModal, setOpenModal] = useState(false);
    const [selectedParent, setSelectedParent] = useState<result | null>(null);
...
<FormModal open={openModal} onClose={() => setOpenModal(false)} result={selectedParent} />
        ...
        <TableBody>
          {resultsData.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell><div className="ml-0">{item.student}</div></TableCell>
              <TableCell><div className="ml-3">{item.score}</div></TableCell>
